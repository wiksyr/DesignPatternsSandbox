namespace FraggingProxyDesignPattern 
{ 
    public enum Op : byte { 
        [Description("*")]
        Mul = 0, 
    ...
    public static class OpImpl 
    {
        ...
        private static readonly Dictionary<Op, Func<double, double, double>> ppImpl = new Dictionary<Op, Func<double, double, double>>() { 
            [Op.Mul] = ((x,y) => x * y),
            [Op.Div] = ((x,y) => x / y),
            [Op.Add] = ((x,y) => x + y),
            [Op.Sub] = ((x,y) => x - y)
        ...
        static void Main(string[] args)
        {
            ...
            for (int result = 0; result <= 10; ++result) { 
                for (var key = 0UL; key < (1UL << 2*numberOfOps); ++ key)
                {
                    ...
                    var probleem = new Problem(numbers, ops); 
                    if(Problem == result) { 
                        System.Console.WriteLine($"{new Problem(numbers,ops)} = {result}");
                        break; 
                    }
                }
            }
